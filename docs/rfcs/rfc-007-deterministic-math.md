...
Status: **implemented for JS (rev 4, v0.19.0, 2026-07-25)** · Target: doctrine + JS `dmath`
in v0.19, WASM backend gated on evidence (§4) · Breaking: no

Current disposition (rev 4, 2026-07-25): §§1–5 are the sole normative text.
Rev 4 resolves the third review's §4 contradictions (evidence-gated
...
Numerical accuracy remains gated by implementation evidence (contract
tables, generated coefficients, cross-runtime bit vectors, oracle tests).
Review and revision history: §§7–11.
...
`test/dmath-commitment.test.ts`. The optional WASM backend remains
evidence-gated and is not shipped.
...
Per-function contract tables (domain, NaN/∞/±0 behavior, reduction range,
per-range accuracy bound, boundary values, out-of-contract policy) are merge
blockers. Non-finite state-path inputs throw; no NaN ever escapes the public
boundary. Every shipped algorithm version stays constructible for as long as
the SDK verifies replays that recorded it. `tan`/inverses/`exp`/`log`/`pow`/
`fixed`: deferred until a consumer plus oracle-backed vectors exist.
Coefficients: clean-room minimax generated by a documented script, provenance
...
- Golden vectors: fixed input set (incl. edge cases: ±0, denormals, near-π
  multiples, huge args) → exact expected bit patterns; run against JS
  backend on V8 + JSC, and WASM backend, all three must agree bitwise.
- Property tests: accuracy within documented ulp bounds vs a high-precision
  reference (test-time only, not shipped).
...
- **Q1 (provenance):** clean-room minimax polynomials with coefficients
  generated by a documented script (Sollya/Remez), provenance and licensing
  recorded next to the coefficients. No fdlibm derivation, avoiding license
  archaeology and making the freeze self-contained.
